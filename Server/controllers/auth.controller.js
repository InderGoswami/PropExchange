...
export const google=async (req,res,next)=>{
    try{
        ...
        if(user){
            ...
            const newUser=new User({username:req.body.name.split(" ").join("").toLoweCase() +Math.random().toString(36).slice(-4),email:req.body.email,password:hashPass,avatar:req.body.photo});
            await newUser.save();
            const token=jwt.sign({id:user._id},process.env.JWT_SECRET);
